...
public class ACOImpl implements ACO {
	...
	public int length(List<Integer> visitedNodes,
		...
		for (int i = 1; i < visitedNodes.size(); i++) {
			...
			for (int j = 0; j < connectionList.size(); j++) {
				...
				// verbindet
				if ((visitedNodes.get(i) == connectionList.get(j).cities.get(0) && visitedNodes
						.get(i - 1) == connectionList.get(j).cities.get(1))
						|| (visitedNodes.get(i) == connectionList.get(j).cities
								.get(1) && visitedNodes.get(i - 1) == connectionList
								.get(j).cities.get(0))) {
	...
	public List<Integer> visitedStreets(List<Integer> visitedNodes,
			List<Connection> connectionList) {
	...
	public List<Ant> createAnts(int antCount, Node startPosition) {
		...
		List<Integer> lastMoves = new ArrayList<Integer>();

		visitedNodes.add(startPosition.ID);

		for (int i = 0; i < antCount; i++) {
			ants.add(new Ant(i + 1, visitedNodes, lastMoves));
		}
	...
	public Ant move(Ant ant, Node node) {
		...
		return new Ant(ant.ID, nodeList, ant.lastTwoCon);
	}
	...
	public Ant clear(Ant ant) {
		...
		return new Ant(ant.ID, nodeList, lastMoves);
	}
	...
	public boolean tourFinished(Ant ant, int numCities) {
		...
		for (int i = 1; i <= numCities; i++) {
			if (!ant.visitedNodes.contains(i)) {
				return false;
			}
		}
	...
	public Connection updatePheromones(Connection oldConnection, double q) {
		...
		return new Connection(oldConnection.ID, oldConnection.length,
				newPheromon, oldConnection.cities);
	}
	...
	public Map<Connection, Double> findPath(Ant ant, Node aktNode,
			int numCities, double alpha, double beta) {
		...
		for (int i = 0; i < aktNode.trails.size(); i++) {
			...
			if (!(ant.visitedNodes.containsAll(currentCities))) {
				...
				nenner += Math.pow(aktNode.trails.get(i).pheromon, alpha)
						* Math.pow(1.0 / aktNode.trails.get(i).length, beta);
		...
		if (ConnectionAnzahl == 0) {
			for (int i = 0; i < tmp.size(); i++) {
				nenner += Math.pow(tmp.get(i).pheromon, alpha)
						* Math.pow(1.0 / tmp.get(i).length, beta);
				double zaehler = Math.pow(tmp.get(i).pheromon, alpha)
						* Math.pow(1.0 / tmp.get(i).length, beta);
		...
		for (Map.Entry<Connection, Double> e : probabilities.entrySet()) {
			...
			Double d = e.getValue();
			if (d == 1.0) {
				double zaehler = Math.pow(c.pheromon, alpha)
						* Math.pow(1.0 / c.length, beta);
	...
	public Node randomPathChoice(Ant a, List<Node> nodes,Map<Connection, Double> probMap, Node startNode) {
		...
		if (a.lastTwoCon.size() == 2) {
			if(probMap.size() == 1){
				entries = sortByValue(probMap);
				Connection con = entries.get(0).getKey();
				resultIndex = con.cities.get(0);
				
				if (resultIndex == startNode.ID) {
					resultIndex = con.cities.get(1);
				}
			}else { 
				if (c.ID == a.lastTwoCon.get(0) && c.ID == a.lastTwoCon.get(1)) {
			
				probMap.remove(c);

				entries = sortByValue(probMap);
				int tmpCon = (int) (Math.random() * entries.size());
				Connection con = entries.get(tmpCon).getKey();
		...
		//letzten zwei Connection aktualisieren
		if (a.lastTwoCon.size() < 2) {
			a.lastTwoCon.add(0, 0);
			a.lastTwoCon.add(1, 0);
		}
		a.lastTwoCon.remove(0);
		a.lastTwoCon.add(1, c.ID);
