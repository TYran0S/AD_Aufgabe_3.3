...
//        capacity = 8;
        ...
        // der
        // Testdatei
		for (int i = 0; i < Parser.packetList.size(); i++) {
			t.get(0).add(i + 2);
			t.get(1).add(Parser.packetList.get(i));
        }
    ...
    public Simulation doSteps(int steps) {
        ...
        if(bestRoute.isEmpty()){
        	JOptionPane.showMessageDialog(null, "Noch keinen Weg gefunden,bitte mehr Steps ausführen!");
        } else {
